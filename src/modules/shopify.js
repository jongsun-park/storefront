...
export const client = Client.buildClient({
  ...
  storefrontAccessToken: process.env.REACT_APP_STOREFRONT,
});
...
export function useShopify() {
  ...
  const cartCount = useSelector((appState) => appState.shopify.cartCount);
  const products = useSelector((appState) => appState.shopify.products);
  const product = useSelector((appState) => appState.shopify.product);
  const featured = useSelector((appState) => appState.shopify.featured);
  const checkoutState = useSelector((appState) => appState.shopify.checkout);
  const shopDetails = useSelector((appState) => appState.shopify.shop);
